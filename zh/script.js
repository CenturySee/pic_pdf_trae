...
function updateImageGrid() {
    ...
    if (uploadedImages.length === 0) {
        imageGrid.innerHTML = `<p class="empty-state">请先上传图片</p>`;
        return;
    }
...
function previewPdf(file) {
    ...
    reader.onload = async function(e) {
        ...
        try {
            ...
            if (isPreviewingPdf) {
                pdfPreview.innerHTML = `<p class="empty-state">转换失败</p>`;
...
function pausePreview() {
    ...
    // 更新预览区域
    const pdfPreview = document.getElementById('pdfPreview');
    pdfPreview.innerHTML = `<p class="empty-state">预览已暂停</p><p>您可以直接点击"转换为图片"按钮进行图片转换</p>`;
    ...
    // 当页数大于2时，使用zip打包
            if (totalPages > 2) {
                ...
                for (let i = 0; i < totalPages; i++) {
                    ...
                    progressFill.style.width = `${progressPercentage}%`;
                    progressText.textContent = `转换中 ${i + 1}/${totalPages}`;
            ...
            try {
                ...
                if (totalPages > 2) {
                    ...
                    for (let pageNum = 1; pageNum <= totalPages; pageNum++) {
                        ...
                        progressFill.style.width = `${progressPercentage}%`;
                        progressText.textContent = `转换中 ${pageNum}/${totalPages}`;
