...
function updateImageGrid() {
    ...
    if (uploadedImages.length === 0) {
        imageGrid.innerHTML = `<p class="empty-state">まず画像をアップロードしてください</p>`;
        return;
    }
        ...
        imageItem.innerHTML = `
            <div style="position: relative; overflow: hidden; height: 150px;">
                ...
                <div style="position: absolute; top: 0.5rem; right: 0.5rem; display: flex; gap: 0.25rem;">
                    <button class="rotate-btn" data-index="${index}" title="画像を回転">&#8635;</button>
                    <button class="delete-btn" data-index="${index}" title="画像を削除">&times;</button>
                </div>
            </div>
...
async function generatePdf() {
    ...
    try {
        ...
        generatePdfBtn.textContent = 'PDFを生成';
...
function previewPdf(file) {
    ...
    // 清空预览区域
    pdfPreview.innerHTML = `<h4>生成中 ${file.name}...</h4>`;
    ...
    reader.onload = async function(e) {
        ...
        try {
            ...
            if (isPreviewingPdf) {
                pdfPreview.innerHTML = `<p class="empty-state">変換に失敗しました</p>`;
    ...
    reader.onerror = function(error) {
        ...
        if (isPreviewingPdf) {
            pdfPreview.innerHTML = `<p class="empty-state">変換に失敗しました</p>`;
            progressText.textContent = '読み込みに失敗しました';
        }
    };
...
function pausePreview() {
    ...
    // 更新预览区域
    const pdfPreview = document.getElementById('pdfPreview');
    pdfPreview.innerHTML = `<p class="empty-state">プレビューが一時停止されました</p><p>直接「画像に変換」ボタンをクリックして画像に変換できます</p>`;
